 ...
 *
 * ✔️ Este arquivo agora expõe duas formas:
 *    - attachRoutes(app): registra as rotas (para Netlify Functions)
 *    - registerRoutes(app): registra as rotas + WebSocket (para servidor Node dedicado)
 *
 * 🧩 Onde mexer para seu caso:
 *   - Upload/Imagens: veja server/modules/products.ts (comentários de Cloudinary/S3)
 *   - Formularios (forms): rotas auth/users/orders/products já aceitam JSON.
 *       Se usar um serviço externo para formulários, aponte o POST desse serviço
 *       para /api/... correspondente (ex.: /api/orders para pedidos).
 *   - Banco de dados: defina DATABASE_URL (Neon) nas variáveis de ambiente do Netlify.
 */
